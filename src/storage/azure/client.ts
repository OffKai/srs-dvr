...
export async function getAzureContainerClient(): Promise<ContainerClient> {
	const containerClient = blobClient.getContainerClient(server.config.DVR_CONTAINER_NAME);

	const exists = await containerClient.exists();
	if (!exists) {
		throw new Error(`Blob container "${server.config.DVR_CONTAINER_NAME}" does not exist`);
	}
